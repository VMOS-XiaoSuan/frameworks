...
void SoftwareRenderer::resetFormatIfChanged(const sp<AMessage> &format) {
    CHECK(format != NULL);
    ALOGE("SHENGXX:isemu=%d resetFormatIfChanged", runningInEmulator());
    ...
    ALOGE("SHENGXX:isemu=%d mColorFormat=%p", runningInEmulator(), mColorFormat);
    switch (mColorFormat) {
    ...
    ALOGE("SHENGXX:2isemu=%d mColorFormat=%p", runningInEmulator(), halFormat);
