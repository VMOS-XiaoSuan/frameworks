...
#ifndef MY_DEBUG_ROOT   //# sheng 设置包的所属 uid:gid
...
static void* rfbConnectOnUnixJavaPort(void*xx)
{
#define UNIX_PATH_MAX 108

    const char *name = "vmosrunstatus";
    size_t socklen;
    size_t namelen;
    ...
    if (socketID == -1) {
        ...
        if(socketID < 0)
        {
            ALOGE("rfbConnectOnUnixJavaPort socket error=%s\n", strerror(errno));
            return (void*)-1;
        }
    }
    ...
    {
        ALOGE("rfbConnectOnUnixJavaPort connect error%s\n", strerror(errno));
        close(socketID);
        socketID = -1;
        return (void*)-1;
    }
    ALOGE("rfbConnectOnUnixJavaPort connect success\n");
           ...
           bool vm_safe_mode, bool is_patchoat)
{
    ...
    out_fd = open(out_path, O_RDWR | O_CREAT | O_EXCL, 0666);
    if (out_fd < 0) {
        ALOGE("installd cannot open '%s' for output during dexopt\n", out_path);
        goto fail;
    }
...
#else
            swap_fd = open(swap_file_name, O_RDWR | O_CREAT | O_EXCL, 0666);
#endif
            if (swap_fd < 0) {
...
// 设置dex进度
    ...
    char prop_buf[PROPERTY_VALUE_MAX];
    //sprintf(prop_buf, "%d:%s", nprogress, input_file);
    sprintf(prop_buf, "%d", nprogress);
    bool profiler = property_set("dalvik.vm.progress", prop_buf);
    ALOGE("profiler %s\n", prop_buf);
#else
    if (nprogress == 0) {
        char szprop[512];
        property_get("svc.progress", szprop, 0);
        nprogress = atoi(szprop);
    }
    if (socketID == -1)
        rfbConnectOnUnixJavaPort(NULL);

    int x = nprogress++;
    x = htonl(x);
    if (socketID != -1) write(socketID, &x, 4);
    else ALOGE("dalvik.vm.progress but not connect socket update=%d\n", x);
...
#else
        mkinnerdirs(dstpath, dstbasepos, 0777,
                2000, 2000, statbuf);
#endif
...
#else
    idmap_fd = open(idmap_path, O_RDWR | O_CREAT | O_EXCL, 0666);
#endif
    if (idmap_fd < 0) {
...
#else
    if (fchown(idmap_fd, 2000, 2000) < 0) {
        ALOGE("idmap cannot chown '%s'\n", idmap_path);
        goto fail;
    }
#endif
...
#endif
        if (flock(idmap_fd, LOCK_EX | LOCK_NB) != 0) {  // 文件锁
