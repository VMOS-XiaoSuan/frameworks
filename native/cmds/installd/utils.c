...
#else
        int fdfd = openat(ddfd, name, O_WRONLY | O_NOFOLLOW | O_CLOEXEC | O_CREAT, 0666);
#endif
        if (fsfd == -1 || fdfd == -1) {
...
#else
        res = _copy_dir_files(sdfd, ddfd, 2000, 2000);
#endif
    } else {
        res = -errno;
    }
...
#else
    if (fs_prepare_dir(media_user_path, 0777, 2000, 2000) == -1) {
#endif
        return -1;
    }
...
#else
    // writable by system, readable by any app within the same user
    const int uid = multiuser_get_uid(userid, 2000);
    const int gid = multiuser_get_uid(userid, 2000);
#endif
...
#else
    if (fs_prepare_dir(config_user_path, 0777, 2000, 2000) == -1) {
#endif
        return -1;
    }
...
#else
    int fd = open(profile_file, O_WRONLY | O_CREAT | O_NOFOLLOW | O_CLOEXEC, 0666);
#endif
