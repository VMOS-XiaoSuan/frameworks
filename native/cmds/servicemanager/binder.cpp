...
extern "C" int  isHostBinderLoad;
extern "C" int   g_DeviceFD;
struct binder_state *binder_open(size_t mapsize)
{
    ...
    // binde2
    bs->fd =  g_DeviceFD;//open("/dev/binder", O_RDWR);
    if (bs->fd < 0) {
        ALOGE("binder: cannot open device (%s)\n",
                strerror(errno));
        goto fail_open;
    }
    ...
    if (!isHostBinderLoad)
    {
        ...
        if (bs->mapped == MAP_FAILED) {
            ALOGE("binder: cannot map device (%s)\n",
                    strerror(errno));
            goto fail_map;
        }
    }
...
static struct flat_binder_object *bio_alloc_obj(struct binder_io *bio)
{
    ...
    obj = (flat_binder_object *)bio_alloc(bio, sizeof(*obj));
...
void bio_put_ref(struct binder_io *bio, uint32_t handle)
{
    ...
    if (handle)
        obj = (flat_binder_object*)bio_alloc_obj(bio);
    else
        obj = (flat_binder_object*)bio_alloc(bio, sizeof(*obj));
...
static struct flat_binder_object *_bio_get_obj(struct binder_io *bio)
{
    ...
    for (n = 0; n < bio->offs_avail; n++) {
        if (bio->offs[n] == off)
            return (flat_binder_object *)bio_get(bio, sizeof(struct flat_binder_object));
    }
